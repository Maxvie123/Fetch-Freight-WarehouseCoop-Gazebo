...
class freight_process():
	...
	def __init__(self,robotname):
		...
		# 	rospy.Subscriber('fetch'+str(picker_id+1)+'/state', std_msgs.msg.Float32, self.get_picker_state, picker_id)
...
class fetch_process():
	...
	transporter_num = 3
	transporter_state = [0,0,0]
	transporter_index = [1,2,3]
	...
	def __init__(self,robotname):
		...
		for transporter_id in (range(transporter_num)):
			rospy.Subscriber('freight'+str(transporter_id+1)+'/state', std_msgs.msg.Float32, self.get_transporter_state, fetch_process.transporter_index[transporter_id])

		# rospy.Subscriber('freight1'+'/state', std_msgs.msg.Float32, self.get_transporter_state, 1)
		# misc varibles 
		self.robotname = robotname
		self.state = 0
		self.start = 0
	...
	def get_transporter_state(self, msg, transporter_id):
		...
		transporter_state[transporter_id] = msg.data
		print "transporter_id {}".format(transporter_id)
		return
